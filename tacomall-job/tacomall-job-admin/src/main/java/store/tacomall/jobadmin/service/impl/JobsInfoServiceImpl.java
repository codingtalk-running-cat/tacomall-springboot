...
import store.tacomall.jobadmin.mapper.JobsInfoMapper;
...
public class JobsInfoServiceImpl implements IJobsInfoService {
    ...
    public R<IPage<JobsInfo>> page(HttpServletRequest request, JobsInfo jobsInfo) {
        return R.ok(jobInfoMapper.selectPage(JobsPageHelper.getPage(request), Wrappers.query(jobsInfo)));
    }
    ...
    public List<JobsInfo> listNextTime(long nextTime) {
        return jobInfoMapper.selectList(
                Wrappers.<JobsInfo>lambdaQuery().eq(JobsInfo::getStatus, 0).le(JobsInfo::getNextTime, nextTime));
    }
